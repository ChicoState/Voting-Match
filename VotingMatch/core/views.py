...
def form_add_user_issue(request, id):
    ...
    issues = Issue.objects.all().exclude(
        name__in=voter.issues.all().values_list("name", flat=True)
    )
...
def form_issue_search(request):
    ...
    results = Issue.objects.filter(name__icontains=search_text).exclude(
        name__in=user_issues.values_list("name", flat=True)
    )
    ...
    for candidate in Candidate.objects.all():
        ...
        try:
            ...
            cand_score = CandidateScore(
                candidate=candidate, voter=voter, score=float(score)
            )
            cand_score.save()
